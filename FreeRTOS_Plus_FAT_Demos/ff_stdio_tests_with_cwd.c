...
/*_RB_ Temporary definition of ff_deltree() as it uses recursion - which is
against coding standards.  This function is only provided for use by
prvFileSystemAccessTask(), hence it is static. */
int ff_deltree( const char *pcPath )
{
FF_FindData_t *pxFindStruct;
int iIsDot, iIsDoubleDot;
	...
	if( ff_findfirst( pcPath, pxFindStruct ) == 0 )
	{
		do
		{
			/* The file was a directory.  Don't traverse '.' or '..'. */
			iIsDot = strcmp( pxFindStruct->pcFileName, "." );
			iIsDoubleDot = strcmp( pxFindStruct->pcFileName, ".." );

			if( ( iIsDot != 0 ) && ( iIsDoubleDot != 0 ) )
			{
				if( ( pxFindStruct->ucAttributes & FF_FAT_ATTR_DIR ) != 0 )
				{
					/* Is the directory empty? */
					if( ff_isdirempty( pxFindStruct->pcFileName ) == pdFALSE )
					{
						/* RECURSIVE CALL!  VERY BAD hence this is a temporary
						function. */
						ff_chdir( pxFindStruct->pcFileName );
						ff_deltree( "." );
						ff_chdir( ".." );
					}

					configASSERT( ff_rmdir( pxFindStruct->pcFileName ) == 0 );
				}
				else
				{
					configASSERT( ff_remove( pxFindStruct->pcFileName ) == 0 );
				}
			}
