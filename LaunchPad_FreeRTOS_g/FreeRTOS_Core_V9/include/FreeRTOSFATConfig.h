 ...
 * FreeRTOSFATConfig.h
 *
 *  Created on: 2015 szept. 17
 *      Author: loszi
 */

#ifndef INCLUDE_FREERTOSFATCONFIG_H_
#define INCLUDE_FREERTOSFATCONFIG_H_

extern void vLoggingPrintf( const char *pcFormatString, ... );

 /* Must be set to either pdFREERTOS_LITTLE_ENDIAN or pdFREERTOS_BIG_ENDIAN, depending on the endian of the architecture on which FreeRTOS is running.
  */
...
/* Set to an index within FreeRTOS's thread local storage array that is free for use by FreeRTOS+FAT. FreeRTOS+FAT will use two consecutive indexes from
 * this that set by ffconfigCWD_THREAD_LOCAL_INDEX. The number of thread local storage pointers provided by FreeRTOS is set by
 * configNUM_THREAD_LOCAL_STORAGE_POINTERS in FreeRTOSConfig.h.
 */
#define ffconfigCWD_THREAD_LOCAL_INDEX 0

/* Set to 1 to include long file name support.
 ...
 * Users should familiarise themselves with any patent issues that may potentially exist around the use of long file names in
 * FAT file systems before enabling long file name support.
 */
 ...
 * The short name will be stored in the 'pcShortName' field of FF_DIRENT. Set to 0 to only include a file's long name.
 * */
#define ffconfigINCLUDE_SHORT_NAME 0

/* Set to 1 to recognise and apply the case bits used by Windows XP+ when using short file names - storing file names
 * such as "readme.TXT" or "SETUP.exe" in a short-name entry. This is the recommended setting for maximum compatibility.
 ...
 * Normally this is quick enough and it is efficient. If ffconfigCACHE_WRITE_THROUGH is set to 1 then buffers will also be flushed
 * each time a buffer is released - which is less efficient but more secure.
 */
 ...
 * When set to 1 all file handles will be "invalidated" if the media is extracted. If set to 0 then file handles will not be invalidated.
...
/* Set to 1 to calculate a HASH value for each existing short file name. Use of HASH values can improve
 * performance when working with large directories, or with files that have a similar name.
 ...
 * rather than having to create one directory in the tree at a time. For example mkdir( "/etc/settings/network", pdTRUE );.
...
/* Set to a function that will be used for all dynamic memory allocations. Setting to pvPortMalloc() will use
 * the same memory allocator as FreeRTOS. For example: #define ffconfigMALLOC( size ) pvPortMalloc( size )
 */
 ...
 */
#define ffconfigDRIVER_BUSY_SLEEP_MS 1
...
/* Force FAT16 on small disk. */
#define ffconfigMIN_CLUSTERS_FAT16 295

/* Sets the maximum length for file names, including the path. Note that the value of this define is directly related to the
 * maximum stack use of the +FAT library. In some API's, a character buffer of size 'ffconfigMAX_FILENAME' will be declared on stack.
 */
#define ffconfigMAX_FILENAME (192 + 1)

/*Ez meg nemtommi - nem találtam semmi dokumentációt hozzá.
 */
//#define ffconfigDEV_SUPPORT 1
//#define ffconfigDEV_PATH "storage"

#define FF_PRINTF 	vLoggingPrintf

#endif /* INCLUDE_FREERTOSFATCONFIG_H_ */
