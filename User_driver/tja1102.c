...
uint8 Phy_init(uint32 phyAddr)
{
    ...
    UartSendString(sciREG1,"\r\n\0");
     ...
     {
         UartSendString(sciREG1," TJA1102S Device ID :  \0");
         UartSendHexU32(sciREG1,id);
         UartSendString(sciREG1,"\r\n\0");
     }
     else if(id ==NXP_PHY_ID_TJA1102P0 )
     {
         UartSendString(sciREG1," TJA1102 Device ID :  \0");
         UartSendHexU32(sciREG1,id);
         UartSendString(sciREG1,"\r\n\0");
     }
    ...
    if ((temp & 0x8000) && (temp !=0xFFFF))
        UartSendString(sciREG1," P0 configure as  :  Master \r\n\0");
    else
        UartSendString(sciREG1," P0 configure as  :  Slave  \r\n\0");

    temp = PhyRead(phyAddr,PHY_BSR);
    if ((temp & 0x0004)&& (temp !=0xFFFF))
        UartSendString(sciREG1," P0 link status   :  Up \r\n\0");
    else
        UartSendString(sciREG1," P0 link status   :  Down \r\n\0");

    temp = PhyRead(phyAddr,PHY_COM_SR);
    UartSendString(sciREG1," P0 reg23 value   :  \0");
    UartSendHexU16(sciREG1,temp);
    UartSendString(sciREG1,"\r\n\0");
    ...
    if( id == NXP_PHY_ID_TJA1102P0 )
    {
        ...
        if ((temp & 0x8000) && (temp !=0xFFFF))
            UartSendString(sciREG1," P1 configure as  :  Master \r\n\0");
        else
            UartSendString(sciREG1," P1 configure as  :  Slave  \r\n\0");

        temp = PhyRead(phyAddr+1,PHY_BSR);
        if ((temp & 0x0004) && (temp !=0xFFFF))
            UartSendString(sciREG1," P1 link status   :  Up \r\n\0");
        else
            UartSendString(sciREG1," P1 link stauts   :  Down \r\n\0");

        temp = PhyRead(phyAddr+1,PHY_COM_SR);
        UartSendString(sciREG1," P1 reg23 value   :  \0");
        UartSendHexU16(sciREG1,temp);
        UartSendString(sciREG1,"\r\n\0");
