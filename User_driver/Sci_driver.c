...
/*
    Uart1_recv_QueHandle = xQueueCreate(1, sizeof(Uart1RecvBuf));
    if(Uart1_recv_QueHandle !=NULL )
    {

        xTaskCreate((TaskFunction_t )Uart1_RecvTask,
                    (const char*    )"Uart1Recv_Task",
                    (uint16_t       )UART1_STK_SIZE,
                    (void*          )NULL,
                    (UBaseType_t    )UART1_TASK_PRIO | portPRIVILEGE_BIT,
                    (TaskHandle_t*  )&Uart1RecvTask_Handler);
        val =(uint8_t) pdTRUE;
    }
    else
    {
        vQueueDelete(Uart1_recv_QueHandle);
        Uart1_recv_QueHandle = NULL;
        val = (uint8_t)pdFALSE;
    }
*/
#if (DEBUG)
        UartSendString(sciREG1," UART1 is init: 115200-8-n-1  \r\n\0");
#endif
    ...
    if(Uart2_recv_QueHandle !=NULL )
    {
        ...
        val =(uint8_t) pdTRUE;
#if (DEBUG)
        UartSendString(sciREG1," UART2 is init: 115200-8-n-1  \r\n\0");
#endif
    ...
    if(Uart3_recv_QueHandle !=NULL )
    {
        ...
        xTaskCreate((TaskFunction_t )Uart3_RecvTask,
                    ...
                    (void*          )NULL,
                    (UBaseType_t    )UART3_TASK_PRIO | portPRIVILEGE_BIT,
                    (TaskHandle_t*  )&Uart3RecvTask_Handler);
        val =(uint8_t) pdTRUE;

#if (DEBUG)
        UartSendString(sciREG1," UART3 is init: 115200-8-n-1  \r\n\0");
#endif
...
#if (DEBUG)
        UartSendString(sciREG1," UART4 is init: 115200-8-n-1  \r\n\0");
#endif
    }
    else
    {
